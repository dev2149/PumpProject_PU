...
public class StartSceneFIO : MonoBehaviour
{
    [SerializeField] private SceneIdx scneneType;
    ...
    private void DivisionScene()
    {
        if (scneneType.Equals(SceneIdx.Title))
        {
            Directory.Instance.sceneController.StartOnMoveTitle(SceneIdx.Title);
        }
    }
}
