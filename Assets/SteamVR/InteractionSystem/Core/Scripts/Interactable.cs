...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Interactable : MonoBehaviour
    {
        ...
        // 추가 비트 연산자로 공구를 잡고 있을경우 다른 레이어
        [SerializeField] private ItemPackageReference itemPackage;

        private void Awake()
        {
            skeletonPoser = GetComponentInChildren<SteamVR_Skeleton_Poser>();
            itemPackage = GetComponent<ItemPackageReference>();
        }
        ...
        protected virtual void OnDetachedFromHand(Hand hand)
        {
            if(this.enabled.Equals(true))
            {
                ...
                if (itemPackage != null)
                {
                    hand.hoverLayerMask.value = 1 << 0;
                    hand.hoverLayerMask.value += 1 << 1;
                    hand.hoverLayerMask.value += 1 << 2;
                    hand.hoverLayerMask.value += 1 << 3;
                    hand.hoverLayerMask.value += 1 << 4;
                    hand.hoverLayerMask.value += 7 << 4;
                    hand.hoverLayerMask.value += 5 << 7;
                    hand.hoverLayerMask.value += 6 << 8;
